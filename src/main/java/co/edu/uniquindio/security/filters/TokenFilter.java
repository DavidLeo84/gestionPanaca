...
//Si la petición es para la ruta /api/clientes se verifica que el token sea correcto y que el rol sea CLIENTE
                ...
                if (requestURI.startsWith("/api/admins")) {
                    if (token != null) {
                        ...
                        if (!jws.getPayload().get("rol").equals("ADMINISTRATOR")) {
                ...
                if (requestURI.startsWith("/api/auxiliares")) {
                    if (token != null) {
                        Jws<Claims> jws = jwtUtils.parseJwt(token);
                        if (!jws.getPayload().get("rol").equals("AUXILIAR")) {
                            crearRespuestaError("No tiene permisos para acceder a este recurso",
                                    HttpServletResponse.SC_FORBIDDEN, response);
                        } else {
                            error = false;
                ...
                if (requestURI.startsWith("/api/receptionists")) {
                    if (token != null) {
                        Jws<Claims> jws = jwtUtils.parseJwt(token);
                        if (!jws.getPayload().get("rol").equals("RECEPTION")) {
                            crearRespuestaError("No tiene permisos para acceder a este recurso",
                                    HttpServletResponse.SC_FORBIDDEN, response);
                        } else {
                            error = false;
